...
5. **充电区阶段**：等待区排到首位后自动进入充电区，进入后请求用户核对充电协议和电量并确认开始充电（超时未确认扣除基础服务费）
...
3. **排队区活动**：
   ...
   - 排队区排到最前时，车辆自动进入等待区（退出0费用）

4. **等待区活动**：
   - 等待区不可更换队列，退出需扣除基础服务费
...
2. **用户动线简化**：排队区→等待区→充电区均为自动流转，无需用户中断确认，减少操作步骤
...
6. **退出计费阶梯化**：排队区退出0费用，等待区和充电区确认前退出扣除基础服务费，保障资源利用率
