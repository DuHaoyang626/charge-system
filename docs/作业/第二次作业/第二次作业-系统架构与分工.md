...
| `TariffConfig计费规则配置` | 计费规则配置值对象 |
| `ChargeMode充电模式`、`PileStatus充电桩状态`、`RequestStatus请求状态` | 枚举类型 |
...
class QueueArea排队区 {
  +capacity: Integer
  +enqueue(request) (入队)
  +dequeueByMode(mode) (按模式出队)
  +changeQueue(requestId, targetMode) (换队)
}

class WaitingArea等待区 {
  +capacity: Integer
  +enterWaiting(request, pile) (进入等待)
  +dequeueToCharging(pile) (出队到充电)
}

class ChargingArea充电区 {
  +parkingSpotsPerPile: Integer
  +findAvailablePiles(mode) (查找可用充电桩)
}
...
class User用户 {
  ...
  +createRequest(vehicle, mode, targetPower) (创建请求)
  +cancelRequest(requestId) (取消请求)
}
...
class Vehicle车辆 {
  ...
  +currentBatteryPercentage: Double
  +chargingProtocol: ProtocolType
}
...
class ChargingRequest充电请求 {
  ...
  +queue_Num: Integer
  +car_Number_before_position: Integer
  +updateRequest(mode, targetPower)
  +confirmEnterWaitingArea()
  +cancel()
}
...
class ChargingPile充电桩 {
  ...
  +TotalChargeTime: Double
  +TotalCapacity: Double
  +estimateCompletionTime(request): DateTime
...
class ChargingSession充电会话 {
  ...
  +startTime: DateTime
  +endTime: DateTime
  +chargedPowerKWh: Double
  +currentPowerKW: Double
  +faultInterrupted: Boolean
  +interruptedPowerKWh: Double
  +modifyTargetPower(targetPowerKWh)
  +end()
  +pause()
  +resume(targetPileId, resumeFromPower)
}
...
class BillingRecord账单记录 {
  ...
  +ChargeAmount: Double
  +ChargeDuration: Double
  +StartTime: DateTime
  +EndTime: DateTime
  +TotalChargeFee: Double
...
class DetailedBill详单记录 {
  ...
  +StartTime: DateTime
  +EndTime: DateTime
  +periodChargeFees: Double[]
  +periodServiceFees: Double[]
  +periodSubtotalFees: Double[]
  +splitByPeriods(tariff)
}
...
class TimeOfUseTariffPolicy分时电价策略 {
  ...
  +normalPrice: Double
  +valleyPrice: Double
  +queryTimeSlotPrice(time): Double
}

class ServiceFeePolicy服务费策略 {
  +baseServiceFee: Double
  +timeCoefficient: Double
  +overtimePenalty: Double
  +calculateServiceFee(durationMinutes, isOvertime): Double
}
...
class TariffConfig计费规则配置 {
  ...
  +normalPrice: Double
  +valleyPrice: Double
  +updateTariff(pileId, peak, normal, valley)
}

class DispatchService调度服务 {
  +assignChargingPile(request): ChargingPile
...
class QueueService队列服务 {
  +moveToQueueArea(request, pile)
  +moveToWaitingArea(request, pile)
  +moveToPileQueue(request, pile)
  +promoteToCharging(pile)
  +getCarState(car_id): CarState
  +getQueueDetail(queuelist): QueueDetail[]
  +changeQueue(vehicleId, fromPile, toPile)
}

class BillingService计费服务 {
  +calculateBill(session, tariffPolicy, serviceFeePolicy): BillingRecord
  +queryBillByDate(carId, date): BillingRecord[]
  +getDetailedBill(Bill_Id): DetailedBill
}

class MonitorService监控服务 {
  +getPileStats(pile_Id): PileStats
  +batchCollectStats(): PileStats[]
...
class Administrator管理员 {
  +adminId: String
  +setPileAvailability(pileId, status)
  +powerOn(pileId)
  +powerOff(pileId)
  +moveVehicle(requestId, targetZone, position)
  +updatePileConfiguration(pileId, maxPowerKW)
  +setParameters(pileId, tariffConfig)
  +startChargingPile(pileId)
  +exportReport()
}
...
| 三区队列（每桩独立） | 每个充电桩的 `PileQueue桩队列` 管理自己的 `QueueArea排队区` + `WaitingArea等待区` |
...
- **后置条件**：
  ...
  2. 对每个充电桩计算 wi（当前排队等待时间）和 ci（预计充电时间）
  3. 计算 Ti = wi + ci
  4. 选择 min(Ti) 对应的充电桩作为目标桩
  5. 车辆进入目标桩排队区
  6. 返回分配结果和预计完成时间
- **异常处理**：
  - 无兼容充电桩：返回无可用资源提示
  - 所有充电桩故障（全部 Ti = ∞）：返回系统繁忙提示
...
| 6、查看充电状态 | Query_Charging_State(car_id) | car_id: 车牌号 | Return(详单信息) |
| 7、结束充电 | End_Charging(car_id, ChargingPileNum) | car_id: 车牌号, ChargingPileNum: 充电桩编号 | Return(0/1) |
| **查看账单** |
| 1、查看账单申请 | Request_Bill(carId, date) | carId: 车牌号, date: 日期 | Return(11个字段) |
| **查看详单（子）** |
| 1、查看详单申请 | Request_DetailedList(Bill_Id) | Bill_Id: 账单编号 | Return(基础信息+分段明细) |
| **运行充电桩** |
| 1、启动充电桩 | powerOn(pile_Id) | pile_Id: 充电桩编号 | Return(0/1) |
| 2、设置参数 | setParameters(pile_Id, 计费规则, 峰时电价, 平时电价, 谷时电价) | pile_Id: 充电桩编号, 电价数据 | Return(0/1) |
| 3、运行充电桩 | Start_ChargingPile(pile_Id) | pile_Id: 充电桩编号 | Return(0/1) |
| 4、关闭充电桩 | powerOff(pile_Id) | pile_Id: 充电桩编号 | Return(0/1) |
| **查看充电桩状态** |
| 1、查看充电桩状态 | Query_PileState(pile_Id) | pile_Id: 充电桩编号 | Return(workingState, TotalChargeNum, TotalChargeTime, TotalCapacity) |
| **查看队列状态** |
| 1、查看队列状态 | Query_QueueState(queuelist) | queuelist: 队列列表 | Return(car_Id, car_Capacity, Request_Amount, waitTime) |
| **管理调度策略** |
| 1、启动默认策略 | initDispatchStrategy(algorithm, faultStrategy) | algorithm: 分配策略标识符, faultStrategy: 故障策略标识符 | 策略就绪 |
| 2、切换分配策略 | switchDispatchStrategy(strategyType) | strategyType: 分配策略标识符 | Return(0/1) |
| 3、切换故障策略 | switchFaultStrategy(faultType) | faultType: 故障策略标识符 | Return(0/1) |
## 四、系统事件人员分配
