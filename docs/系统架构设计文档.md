...
- **Logger 例外**：Logger 是基础设施层公共服务，L2 应用层和 L3 领域层**可依赖** Logger 记录日志，这属于基础设施能力调用，不违反分层禁止规则

**配置读取关系**（`application.yml` → 各模块）：

| 配置区段 | 读取方 | 用途 |
|----------|--------|------|
| `dispatch.*` | DispatchStrategy | 初始化/切换调度策略 |
| `billing.*` | BillingService | 读取电价、服务费规则、超时配置 |
| `logging.console_threshold` | Logger | 判定日志是否输出到终端 |
| `station.*` | 系统启动时 Router | 初始化充电站参数、三区容量 |
| `piles[].*` | 系统启动时 | 注册充电桩到 charging_piles 表 |
| `system.*` | 系统启动时 | 数据库路径、JWT 密钥 |
| `monitoring.*` | MonitorService | 轮询刷新间隔 |
...
| 模块 | 类名 | 职责 | 关键方法 |
|------|------|------|----------|
| 日志服务 | `Logger` | 统一日志记录：写入 DB + 按阈值输出终端 | log(), info(), notice(), warn(), error(), critical() |
| 配置管理 | `ConfigLoader` | 读取 application.yml，提供运行时配置访问 | load(), reload(), get() |
| 数据访问 | `Database` | SQLAlchemy ORM 会话管理 | get_session(), init_db(), migrate() |
...
详细的数据库表结构见 `docs/数据设计说明.md`。系统共使用 **13 张数据库表**：

| 类别 | 表名 | 职责 |
|------|------|------|
| 用户与车辆 | `users`, `vehicles` | 用户账号、车辆参数 |
| 充电桩 | `charging_piles`, `pile_protocols`, `pile_tariff_configs` | 桩状态、协议、独立计费规则 |
| 充电业务 | `charging_requests`, `charging_sessions` | 请求与三区队列、会话记录 |
| 计费支付 | `billing_records`, `detailed_bills`, `payment_orders` | 账单、详单、支付 |
| 监控与日志 | `fault_events`, `system_logs`, `dispatch_strategy_logs` | 故障事件、系统日志、策略变更 |

> 配置与数据分离原则：充电桩硬件参数、容量、时段定义等静态配置存在 `config/application.yml`，运行时状态和业务记录存在数据库。

### 3.6 日志系统设计

日志系统贯穿全系统，详细设计见 `docs/数据设计说明.md §七`。核心要点：

- **5 级日志**：0=常规, 1=注意, 2=警告, 3=严重, 4=崩溃
- **统一入口**：`Logger` 基础设施服务，所有模块通过同一实例记录
- **双通道输出**：全部写入 `system_logs` 表；高于 `logging.console_threshold` 的同时输出到终端
- **10 个模块**：SYSTEM, ACCOUNT, DISPATCH, QUEUE, BILLING, MONITOR, ADMIN, FAULT, SCHEDULER, API
...
2. **排队区可换队**：自由更换到其他充电桩队列队尾（退出0费用）
3. **等待区不可换队**，退出需扣除基础服务费
4. **充电中修改电量**：下限为当前已充电量
5. **充电区确认前退出或超时**：未确认开始充电退出或超时，扣除基本服务费x（可配置）
6. **排队区退出**：0费用
7. **充电桩故障**：需重新调度桩上所有车辆
...
├── 数据设计说明.md                     # 数据库表结构、配置文件详设、Logger 设计
└── 云平台文件/
...
| 阶段 | 内容 | 优先级 |
|------|------|--------|
| 1 | **数据库初始化 + Logger 基础设施** | 必须先建 |
| 2 | **领域模型实体 + 枚举 + 值对象** | 必须先建 |
| 3 | **账号服务 + REST 路由** | 基础能力 |
| 4 | **充电申请 + 三区队列管理** | 核心流程 |
| 5 | **调度策略框架（默认单次最短）** | 调度核心 |
| 6 | **充电会话 + 开始/结束充电** | 充电操作 |
| 7 | **计费服务 + 账单/详单** | 计费核心 |
| 8 | **管理员功能（充电桩管理/参数设置）** | 管理功能 |
| 9 | **五种故障策略** | 容错能力 |
| 10 | **策略切换机制（启动参数 + 运行时）** | 策略管理 |
| 11 | **定时轮询 + 状态监控** | 运维功能 |
...
| 日志记录 | **Logger 基础设施服务** | 统一日志接口，5级分类，DB+终端双通道输出，全模块覆盖（见 `docs/数据设计说明.md §七`） |
