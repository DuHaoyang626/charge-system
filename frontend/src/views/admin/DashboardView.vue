    ...
    <div class="chart-grid">
      <div class="glass-card chart-card chart-span-2">
        <div class="chart-header">
          ...
          <span class="glass-badge">逐日</span>
        </div>
      ...
      <div class="glass-card chart-card">
        <div class="chart-header">
          ...
          <span class="glass-badge">近30天</span>
        </div>
        ...
        <div v-for="s in stationData" :key="s.id" class="station-block">
          <div class="station-block-header">
            <div class="station-block-title">
              ...
              <el-tag :type="s.status === 'running' ? 'success' : s.status === 'stopping' ? 'warning' : 'info'" size="small" effect="plain">
                {{ s.status === 'running' ? '运行中' : s.status === 'stopping' ? '停止中' : '已停止' }}
              </el-tag>
              <span class="station-metrics">🚶{{ s.queueCount }}/{{ s.queueCapacity }} · ⏳{{ s.waitingCount }}/{{ s.waitingCapacity }} · ⚡{{ s.chargingCount }}/{{ s.chargingCapacity }}</span>
            </div>
            <el-tag type="info" size="small" effect="plain" v-if="s.estimatedWaitMinutes">预估等待 {{ s.estimatedWaitMinutes }} 分钟</el-tag>
          </div>
              ...
              <table class="zone-table" v-if="s.chargingList.length">
                ...
                <tbody>
                  <tr v-for="q in s.chargingList" :key="q.sessionId">
                    ...
                    <td><el-progress :percentage="q.progress" :stroke-width="8" :width="80" /></td>
                  </tr>
                </tbody>
              </table>
              <div v-else class="zone-empty">空闲</div>
...
// 后端返回: volume = { totalEnergyKwh, totalSessions, dataPoints: [{period, energyKwh, sessions}] }
const trendData = computed(() => volume.value?.dataPoints?.length ? volume.value.dataPoints : null)
const utilData = computed(() => rawStations.value.length ? rawStations.value : null)
const revenueData = computed(() => revenue.value)

function startPolling() { stopPolling(); pollingTimer = setInterval(() => fetchData(true), POLLING_INTERVAL) }
...
const metrics = computed(() => {
  ...
  return [
    ...
    { label: '排队等待', value: String(queueing), sub: `${rawStations.value.filter((s: any) => s.queueCount > 0).length} 个桩有排队`, color: 'var(--color-warning)', icon: '⏳', iconBg: 'rgba(217,119,6,0.10)' },
    { label: '收入(已支付)', value: rev?.totalRevenue != null ? `¥${rev.totalRevenue}` : '--', sub: `电费 ¥${rev?.electricityRevenue || 0}`, color: 'var(--color-primary)', icon: '💰', iconBg: 'rgba(37,99,235,0.10)' },
  ]
})

const stationData = computed(() => {
  if (stationDetails.value.length > 0) return stationDetails.value
  return rawStations.value.map((s: any) => ({ ...s, queueList: [], waitingList: [], chargingList: [] }))
})

// ── 渲染 ECharts ──
function renderCharts() {
  renderTrendChart(); renderPieChart(); renderRevenueChart(); renderStatusChart()
}
...
function renderTrendChart() {
  ...
  trendChart.setOption({
    ...
    xAxis: { type: 'category', data: labels, axisLine: { lineStyle: { color: 'var(--border-color)' } }, axisLabel: { color: 'var(--text-tertiary)', fontSize: 11 } },
    yAxis: { type: 'value', name: 'kWh', nameTextStyle: { color: 'var(--text-tertiary)', fontSize: 11 }, splitLine: { lineStyle: { color: 'var(--border-light)', type: 'dashed' } }, axisLabel: { color: 'var(--text-tertiary)', fontSize: 11 } },
    series: [{ type: 'line', data: energyData, smooth: true, lineStyle: { width: 3, color: '#3B82F6' }, areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: 'rgba(59,130,246,0.3)' }, { offset: 1, color: 'rgba(59,130,246,0.02)' }]) }, itemStyle: { color: '#3B82F6' }, symbol: 'circle', symbolSize: 6 }],
  })
}
...
function renderPieChart() {
  ...
  const pieData = stations.slice(0, 8).map((s: any) => {
    ...
    return { name: s.name, value: Math.round((used / total) * 100) }
  })
  const colors = ['#3B82F6', '#22C55E', '#F59E0B', '#EF4444', '#8B5CF6', '#EC4899', '#06B6D4', '#84CC16']
  pieChart.setOption({
    tooltip: { trigger: 'item', formatter: '{b}: {c}%' },
    series: [{ type: 'pie', radius: ['35%', '65%'], avoidLabelOverlap: true, itemStyle: { borderRadius: 6 }, label: { show: true, formatter: '{b}\n{d}%', fontSize: 10, color: 'var(--text-secondary)' }, labelLine: { lineStyle: { color: 'var(--border-color)' } }, data: pieData, color: colors }],
  })
}
...
function renderRevenueChart() {
  ...
  revenueChart.setOption({
    ...
    xAxis: { type: 'category', data: barData.map(d => d.name), axisLine: { lineStyle: { color: 'var(--border-color)' } }, axisLabel: { color: 'var(--text-tertiary)', fontSize: 12 } },
    yAxis: { type: 'value', name: '金额 (¥)', nameTextStyle: { color: 'var(--text-tertiary)', fontSize: 11 }, splitLine: { lineStyle: { color: 'var(--border-light)', type: 'dashed' } }, axisLabel: { color: 'var(--text-tertiary)', fontSize: 11, formatter: '¥{value}' } },
    series: [{ type: 'bar', data: barData.map(d => d.value), barWidth: '50%', itemStyle: { borderRadius: [6, 6, 0, 0], color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: '#3B82F6' }, { offset: 1, color: '#1D4ED8' }]) } }],
  })
}
...
function renderStatusChart() {
  ...
  if (!stations || !stations.length) return
  const statusData = [
    stations.filter((s: any) => s.status === 'running').length,
    stations.filter((s: any) => s.status === 'stopped' || s.status === 'stopping').length,
    stations.filter((s: any) => s.status === 'error').length,
  ]
  const items: any[] = []
  if (statusData[0] > 0) items.push({ name: '运行中', value: statusData[0], itemStyle: { color: '#22C55E' } })
  if (statusData[1] > 0) items.push({ name: '已停止', value: statusData[1], itemStyle: { color: '#94A3B8' } })
  if (statusData[2] > 0) items.push({ name: '异常', value: statusData[2], itemStyle: { color: '#EF4444' } })
  if (!items.length) return
  statusChart.setOption({
    tooltip: { trigger: 'item', formatter: '{b}: {c} ({d}%)' },
    series: [{ type: 'pie', radius: ['45%', '70%'], avoidLabelOverlap: true, itemStyle: { borderRadius: 4 }, label: { show: true, formatter: '{b}\n{c}', fontSize: 12, color: 'var(--text-secondary)' }, labelLine: { lineStyle: { color: 'var(--border-color)' } }, data: items }],
  })
}
...
async function fetchData(silent = false) {
  ...
  try {
    ...
    rawStations.value = (staRes.data as any).data?.stations || []

    const promises = rawStations.value.map(async (s: any) => {
      try { const r = await getStationDetailApi(s.id); return (r.data as any).data || s }
    ...
    rawStations.value = []; stationDetails.value = []
  } finally { loading.value = false }
}

function handleResize() { trendChart?.resize(); pieChart?.resize(); revenueChart?.resize(); statusChart?.resize() }

onMounted(() => { fetchData(); startPolling(); window.addEventListener('resize', handleResize) })
onUnmounted(() => {
  stopPolling(); window.removeEventListener('resize', handleResize)
  trendChart?.dispose(); pieChart?.dispose(); revenueChart?.dispose(); statusChart?.dispose()
})
</script>
...
@media (max-width: 600px) { .chart-span-2 { grid-column: span 1; } }

.metric-card { display: flex; align-items: center; gap: clamp(12px, 1.5vw, 16px); padding: clamp(14px, 2vw, 20px); transition: all 0.3s ease; }
.metric-card:hover { transform: translateY(-2px); box-shadow: var(--shadow-lg), var(--shadow-glass); }
.metric-icon { width: clamp(40px, 5vw, 48px); height: clamp(40px, 5vw, 48px); border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; font-size: clamp(20px, 2.5vw, 24px); flex-shrink: 0; }
...
.chart-card { padding: clamp(14px, 2vw, 20px); position: relative; }
...
.zone-col-header { font-size: clamp(12px, 1.1vw, 13px); font-weight: 600; padding: 8px 12px; border-left: 3px solid; border-bottom: 1px solid var(--border-light); }
...
.zone-empty { padding: 20px; text-align: center; color: var(--text-tertiary); }
...
.insight-empty { text-align: center; padding: 32px; color: var(--text-tertiary); }
</style>
